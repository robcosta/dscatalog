...
public class UserService {
	...
	public Page<UserDTO> searchAll(Pageable pageable) {
		...
		return result.map(x -> new UserDTO(x, x.getRoles()));
	}
	
//	@Transactional(readOnly = true)
//	public Page<UserDTO> findAll(Pageable pageable) {
//		Page<User> result = repository.findAll(pageable);
//		return result.map(x -> new UserDTO(x, x.getCategories()));
//	}
	...
	public UserDTO findById(Long id) {
		...
		return new UserDTO(result, result.getRoles());
	}
	...
	public UserDTO insert(UserDTO dto) {
		...
		return new UserDTO(repository.save(entity), entity.getRoles());
	}
